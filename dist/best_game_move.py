...
def compute_across_cross_checks(game_board, board_size=15):
    '''
    ...
    Parameter {int} board_size the size of the board.
    ...
    for row in range(board_size):
        row_cross_checks = [[] for _ in range(board_size)]

        for i in range(board_size): # Iterate over the columns.
            ...
            j = row + 1
            while j != board_size and game_board[j][i] != ' ':
...
def compute_anchors(game_board, board_size=15):
    '''
    ...
    the center cell is set as the only anchor.
    ...
    no_anchors = True
    anchors = [[] for _ in range(board_size)]
        ...
        for j in range(board_size):
            if game_board[i][j] == ' ':                     # Empty cell.
                if j != 0 and game_board[i][j - 1] != ' ':  # Letter to the left.
                    ...
                    anchors[i].append(1)
                    continue
                if j != board_size - 1 and game_board[i][j + 1] != ' ': # Letter to the right.
                    no_anchors = False
                    anchors[i].append(1)
                    continue
                if i != 0 and game_board[i - 1][j] != ' ':  # Letter above.
                    ...
                    anchors[i].append(1)
                    continue
                if i != board_size - 1 and game_board[i + 1][j] != ' ': # Letter below.
                    no_anchors = False
                    anchors[i].append(1)
                    continue
            anchors[i].append(0)
    ...
    if no_anchors:
        center = board_size // 2
        anchors[center][center] = 1

    return anchors

def populate_game_board(letters, board_size=15):
    '''
    Given an letters with their corresponding indices, create the NxN
    WWF game board.

    Parameter {Array<dict>} letters the game letters with letter and index
    key-value pairs.
    Parameter {int} board_size the size of the board (default 15).
    Returns {Array<Array<str>>} the 2d populated gameboard.
    '''

    game_board = []

    # Create an empty NxN game board.
    for i in range(board_size):
        row = []
    ...
    for letter in letters:
        i = int(letter['index'] / board_size)
        j = letter['index'] % board_size
...
def compute(json_data):
    ...
    # Get board size and bonus board (with defaults for backwards compatibility)
    BOARD_SIZE = json_data.get('boardSize', 15)
    CUSTOM_BONUS_BOARD = json_data.get('bonusBoard', None)

    # TODO: Full 11x11 support requires refactoring the entire algorithm below
    # For now, only 15x15 is fully supported
    if BOARD_SIZE != 15:
        return {
            ...
            'score': 0,
            'direction': 'across',
            'error': f'{BOARD_SIZE}x{BOARD_SIZE} board size coming soon! Currently only 15x15 is supported for move calculation.'
        }

    # Populate the user's letter rack and the game board.
    RACK = json_data['userLetters']
    GAME_BOARD = populate_game_board(json_data['gameLetters'], BOARD_SIZE)

    # Use custom bonus board if provided, otherwise use default
    if CUSTOM_BONUS_BOARD:
        GAME_BOARD_BONUSES_TO_USE = CUSTOM_BONUS_BOARD
    else:
        GAME_BOARD_BONUSES_TO_USE = GAME_BOARD_BONUSES

    # Compute the anchors and cross checks.
    anchors = compute_anchors(GAME_BOARD, BOARD_SIZE)
    across_cross_checks = compute_across_cross_checks(GAME_BOARD, 15)  # Still using 15 for now
